...
# bbox(bounding box)を抽出するスクリプト
#
# 使い方: python extract_bbox_by_image_list.py
# オプション: [-i 画像リストファイル]
#             [-o 出力先ディレクトリ]
#             [-s オブジェクト名(synsets)]
#             [-l オブジェクトID]
# 出力: (ID) x y width height
#
# * オプションを指定しない場合は以下の通り.
    ...
    for jl in jpg_list:
        ...
        for jd in json_data_list:
            ...
            if jpg_name == jd_list_id:
                ...
                samples_col.append(jd[1]["x"])  # x
                samples_col.append(jd[1]["y"])  # y
                samples_col.append(jd[1]["w"])  # width
                samples_col.append(jd[1]["h"])  # height
